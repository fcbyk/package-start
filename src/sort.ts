...
// 选择排序
export function selectSort<T>(objectArray: T[], byAttribute: keyof T): SortedResult{
...
// 插入排序
export function insertSort<T>(objectArray: T[], byAttribute: keyof T): SortedResult{

    // 空数组或之有一个元素的数组直接返回
    if(objectArray == null || objectArray.length < 2) return new SortedResult(objectArray);
...
// 冒泡排序
export function bubbleSort<T>(objectArray: T[], byAttribute: keyof T): SortedResult{
...
// 希尔排序
export function shellSort<T>(objectArray: T[], byAttribute: keyof T): SortedResult {
